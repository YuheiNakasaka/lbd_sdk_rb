  ...
  describe '#generate' do
    context 'when query params and request body are empty' do
      it 'should return valid signature' do
        ...
        expect(instance.generate(secret: secret, method: method, endpoint_path: path, timestamp: timestamp,
                                 nonce: nonce,)).to eq(expected)
      end
    end
    ...
    context 'when only request body is empty' do
      it 'should return valid signature' do
        ...
        expected = 'fasfnqKVVClFam+Dov+YN+rUfOo/PMZfgKx8E36YBtPh7gB2C+YJv4Hxl0Ey3g8lGD0ErEGnD0gqAt85iEhklQ=='
        expect(instance.generate(secret: secret, method: method, endpoint_path: path, timestamp: timestamp, nonce: nonce,
                                 query_params: query_params,)).to eq(expected)
      end
    end
    ...
    context 'when only request params is empty' do
      it 'should return valid signature' do
        ...
        expected = '4L5BU0Ml/ejhzTg6Du12BDdElv8zoE7XD/iyOaZ2BHJIJG0SUOuCZWXu0YaF4i4C2CFJhjZoJFsje4CJn/wyyw=='
        expect(instance.generate(secret: secret, method: method, endpoint_path: path, timestamp: timestamp, nonce: nonce,
                                 body: body,)).to eq(expected)
      end
    end
    ...
    context 'when only request params is empty and request body has nested values.' do
      it 'should return valid signature' do
        ...
        expected = 'vhr5c3y2PAP5rmt+4YN1ojbMnT9IkYnIIB1yvWYM9OdECB2Y11fGTLDLRybB3lLKv0kvJQMAelSkQYBKdhSXbg=='
        expect(instance.generate(secret: secret, method: method, endpoint_path: path, timestamp: timestamp, nonce: nonce,
                                 body: body,)).to eq(expected)
      end
    end
